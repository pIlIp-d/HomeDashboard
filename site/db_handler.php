...
function request_handling($request_name, $dbcon, $json_decoded): void
{
    switch ($request_name) {
        ...
        case "save_note":
            ...
            if (isset($note->id)) {
                $stmt = $dbcon->prepare("UPDATE `notes` SET `heading` = :heading, `id` = :id, `text` = :text WHERE `id` = :id OUTPUT INSERTED.[id] INTO @inserted_id");
                echo $note->id;
            } else {
                $stmt = $dbcon->prepare("INSERT INTO `notes` (`heading`,`id`,`text`) VALUES (:heading, :id, :text);");
            }
            $stmt->bindParam(":heading", $heading);
            $stmt->bindParam(":text", $text_value);
